...
describe Guard::Dsl do
  ...
  describe '#ignore!' do
    ...
    it 'replaces ignored regexps in the listener' do
      ...
      subject.instance_eval('ignore! %r{^foo}, /bar/')
    end
    ...
    it 'replaces ignored regexps in the listener but keeps these setted by filter!' do
      ...
      subject.instance_eval("filter! %r{.txt$}, /.*.zip/\n ignore! %r{^foo}")
    end
  end
  ...
  describe '#filter!' do
    ...
    it 'replaces ignored regexps in the listener but keeps these setted by ignore!' do
      ...
      subject.instance_eval("ignore! %r{^foo}\n filter! %r{.txt$}, /.*.zip/")
    end
  end
  ...
  describe '#notification' do
    ...
    it 'adds a notification to the notifier' do
      ...
      subject.instance_eval('notification :growl')
    end
  ...
  describe '#interactor' do
    ...
    it 'disables the interactions with :off' do
      subject.instance_eval('interactor :off')
      expect(Guard::Interactor.enabled).to be_falsey
    end

    it 'passes the options to the interactor' do
      subject.instance_eval('interactor option1: \'a\', option2: 123')
  ...
  describe '#group' do
    ...
    context 'no plugins in group' do
      it 'displays an error' do
        ...
        subject.instance_eval(guardfile_string_with_empty_group)
      end
    end

    context 'group named :all' do
      it 'raises an error' do
        expect { subject.instance_eval("group :all") }.to raise_error(ArgumentError, "'all' is not an allowed group name!")
      end
    end
    ...
    it 'evaluates all groups' do
      ...
      subject.instance_eval(valid_guardfile_string)
    end
    ...
    it 'accepts multiple names' do
      ...
      subject.instance_eval('group :foo, :bar do; end')
    end
  end
  ...
  describe '#guard' do
    ...
    it 'loads a guard specified as a quoted string from the DSL' do
      ...
      subject.instance_eval('guard \'test\'')
    end
    ...
    it 'receives options when specified, from normal arg' do
      ...
      subject.instance_eval('guard \'test\', opt_a: 1, opt_b: \'fancy\'')
    end

    it 'respects groups' do
      expect(::Guard).to receive(:add_plugin).with(:test, { watchers: [], callbacks: [], group: :bar })

      subject.instance_eval('group :foo do; group :bar do; guard :test; end; end')
    end
    ...
    it 'uses :default group by default' do
      ...
      subject.instance_eval('group :foo do; group :bar do; guard :test; end; end; guard :rspec')
    end
  end
  ...
  describe '#logger' do
    ...
    context 'with valid options' do
      it 'sets the logger log level' do
        subject.instance_eval('logger level: :error')
      ...
      it 'sets the logger except filter from an array of symbols and string' do
        subject.instance_eval('logger except: [:rspec, \'cucumber\', :jasmine]')
    ...
    context 'with invalid options' do
      context 'for the log level' do
        it 'shows a warning' do
          ...
          subject.instance_eval('logger level: :baz')
        end
      ...
      context 'when having both the :only and :except options' do
        it 'shows a warning' do
          ...
          subject.instance_eval('logger only: :jasmine, except: :rspec')
        end
  ...
  describe '#scope' do
    ...
    it 'does use the DSL scope plugin' do
      subject.instance_eval('scope plugin: :baz')
